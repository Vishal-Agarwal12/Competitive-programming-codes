    ...
    for(int i = hh;i<h;i++) {
        if(i==hh) {
            for(int j=mm;j<m;j++) {
                ...
                string tt = to_string(j);
                if(tt.size() == 1) tt = "0"+tt;
                
                if(ss[0] == '3' || ss[0] == '4' || ss[0] == '6' || ss[0] == '7' || ss[0] == '9'
                || ss[1] == '3' || ss[1] == '4' || ss[1] == '6' || ss[1] == '7' || ss[1] == '9' 
                || tt[0] == '3' || tt[0] == '4' || tt[0] == '6' || tt[0] == '7' || tt[0] == '9'
                || tt[1] == '3' || tt[1] == '4' || tt[1] == '6' || tt[1] == '7' || tt[1] == '9') continue;

                for(int i=0;i<2;i++) {
                    if(ss[i] == '2') ss[i] = '5';
                    else if(ss[i] == '5') ss[i] = '2';

                    if(tt[i] == '2') tt[i] = '5';
                    else if(tt[i] == '5') tt[i] = '2';
                }

                ll min = (ss[1] - '0')*10 + (ss[0] - '0');
                ll hr = (tt[1] - '0')*10 + (tt[0] - '0');

                if(hr < h && hr >= hh && min >= mm && min < m) {
                    cout<<i<<":"<<j<<endl;
                    return;
                }
            }
